...
class ContactForm(forms.Form):
    ...
    def save(self, fail_silently=False):
        ...
        """
        # If there's a photo attached and a bride's last name, use this save ... 
        if self.is_multipart() and self.cleaned_data['bride_name_last']:
            photo = self.request.FILES['photo']
            orig_name = photo.name
            orig_file_basename, orig_file_ext = os.path.splitext(orig_name)
            
            temp_photo_file = open('/rgcalendar/oper/tmp/tmp_wedding_photo.jpg', 'wb+')
            for chunk in photo.chunks():
                temp_photo_file.write(chunk)
            temp_photo_file.close()
            
            message_dict = self.get_message_dict()
            subject = message_dict['subject']
            body = message_dict['message']
            from_email = message_dict['from_email']
            to = message_dict['recipient_list']
            bride = self.cleaned_data['bride_name_last'].lower()
            groom = self.cleaned_data['groom_name_last'].lower()
            photo.name = 'wd.%s-%s.adv%s' % (bride, groom, orig_file_ext)
            
            info = IPTCInfo('/rgcalendar/oper/tmp/tmp_wedding_photo.jpg', force=True)
            info.data['special instructions'] = 'j=%s-%s d=Features c=Oregon Life s=BW p=07 Sunday' % (bride, groom)
            info.saveAs('/rgcalendar/oper/tmp/tmp_wedding_photo.jpg')
            
            written_photo = open('/rgcalendar/oper/tmp/tmp_wedding_photo.jpg', 'r')
            
            mail = EmailMessage(subject, body, from_email, to)
            mail.attach(photo.name, written_photo.read(), photo.content_type)
            mail.send()
            
            written_photo.close()
            
            # message to the visitoro confirming that form was received
            success_sbj = '%s-%s wedding announcement submission successful' % (self.cleaned_data['bride_name_last'], self.cleaned_data['groom_name_last'])
            success_msg_template = loader.get_template('contact_form/contact_form_wedding_confirmation_email.txt')
            success_msg_context = Context({ 'bride_name_last': self.cleaned_data['bride_name_last'],
                                        'groom_name_last': self.cleaned_data['groom_name_last'],
                                        'photo_name': orig_name, })
            success_msg = success_msg_template.render(success_msg_context)
            success_mail = EmailMessage(success_sbj, success_msg, 'noreply@registerguard.com', to=[from_email], bcc=['john.heasly@registerguard.com','rob.denton@registerguard.com',])
            success_mail.send(fail_silently=False)
            
        # ... otherwise, use this save.
        else:
